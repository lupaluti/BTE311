...
function startGame() {
  ...
  for (let i = 0; i < shuffledDeck.length; i++) {
    ...
    liTag.addEventListener("click", (event) => {
      liTag.style.transform = `rotateY(180deg) rotateX(0deg)`;
    });
...
function AddStats() {
  ...
  p[0].innerHTML = `Geçen süre: ${minutes} dakika ve ${seconds} saniye (En İyi Süre: ${best_minute} dakika ve ${best_second} saniye)`;
  p[1].innerHTML = `Toplam hareket: ${moves} (En İyi Hareket Sayısı: ${best_moves})`;
  p[2].innerHTML = `Yıldız Sayın: ${starCount}/3`;
}
